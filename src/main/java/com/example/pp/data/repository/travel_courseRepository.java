...
import com.example.pp.data.entity.travel_course;
...
public interface travel_courseRepository extends JpaRepository<travel_course, String> {
    @Query(value = """
    SELECT a.id AS id,
           a.name AS name,
  ...
  """, nativeQuery = true)
    List<travel_course> findEntitiesWithinRadius(@Param("lat") double lat,
                                                 @Param("lon") double lon,
                                                 @Param("radiusMeters") double radiusMeters);
}
