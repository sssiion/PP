...
- **Endpoint**: `/logout`
- **Method**: `POST`
- **설명**: 현재 로그인된 사용자를 로그아웃 처리하고, 서버의 세션을 즉시 무효화(삭제)합니다. 세션이 삭제되면 저장된 모든 데이터(예: 추천 결과)도 함께 삭제됩니다.

- **요청 예시**:
  ```bash
  # 세션 유지를 위해 사용했던 쿠키와 함께 요청해야 합니다.
  curl -X POST http://localhost:8082/logout -b "cookies.txt"
  ```
